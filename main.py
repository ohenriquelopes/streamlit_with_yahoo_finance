...
tickerSymbol = st.sidebar.text_input("Ticker Symbol", 'AAPL')
st.sidebar.markdown("Search fro stock ticker symbol for any company in [Yahoo Finance](https://finance.yahoo.com/lookup/)")


# Web scraping of Stock data
tickerData = yf.Ticker(tickerSymbol)
tickerDf = tickerData.history(period='1d', start=start_date, end=end_date)
ticker_data = yf.download(tickerSymbol, start=start_date, end=end_date)
ticker_data.reset_index(inplace=True)

# Ticker information
# string_logo = '<img src=%s>' % yf.Ticker(tickerSymbol).info['logo_url']
...
st.subheader(f'Stock price for {days} days')
...
st.subheader(f'Stock volume for {days} days')
